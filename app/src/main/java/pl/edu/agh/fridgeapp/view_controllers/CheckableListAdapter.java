...
import pl.edu.agh.fridgeapp.model.CheckableListElement;
...
public class CheckableListAdapter extends BaseExpandableListAdapter {
    ...
    public void updateOnCheck(){
        ...
        context.getFridge().setOwnerFilter(getSelectedOwners());
    }
    ...
    public List<User> getSelectedOwners(){
        ...
        for(CheckableListElement child: children.get(2)){
            if(child.isChecked())
                selected.add(new User(child.getName()));
        }
        return selected;
    }
